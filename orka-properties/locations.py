# Source: https://github.com/CSAILVision/places365/blob/master/categories_places365.: txt
import json
import yaml
import os
...
def process_labels(labels_dict):
    label_names = []
    for path, idx in labels_dict.items():
        parts = path.split('/')
        name = parts[-2].replace('_', ' ') if len(parts) == 4 else parts[-1].replace('_', ' ')
        print(name)
        label_names.append(name)
    return list(set(label_names))

def save_labels(labels_dict, filename, format='yaml'):
    ...
    with open(full_path, 'w', encoding='utf-8') as f:
        if format == 'json':
            ...
            yaml.dump(labels_dict, f, sort_keys=False, allow_unicode=True)
        else:
            raise ValueError("Unsupported format. Use 'json' or 'yaml'.")

def load_labels(filename, format='yaml'):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    full_path = os.path.join(script_dir, filename)
...
if __name__ == "__main__":
    processed = process_labels(original_labels)
    save_labels(processed, 'locations.yaml', format='yaml')
    labels_list = load_labels('locations.yaml', format='yaml')
    print(labels_list[:10])  # Sanity check